...
var removeDuplicates = () => {
  var newWork = new Set(workplaceAccidents);
  return Array.from(newWork);
...
var recordCleaner = () => {
  for (let thing in myCar){
    if (thing === "accidents"){
      for (let j = 0; j < myCar[thing].length; j++){
        myCar[thing][j].atFaultForAccident = false;
      }
    }
